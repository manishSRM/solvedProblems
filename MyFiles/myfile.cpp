...
2. call by refernce : when the values of argument are passed by refernce or passing their address called call by value.
    example:
         ...
         here, value of p and q are passed to the a and b res. by thier address so when we change the value of a, b then it actually change the value of p, q and we call it the changes are back reflect.
...
==> example: 
    ...
    reason: when we want changes then we use & operator. By this, here we really want to initialize the variables of structure variable p.                     
...
here, we dosn't want to change the values of variable stucture variable a and b. 
...
==> In this statemant a memory of space int type allocated in the heap section and the address of this memory is saved in a pointer variable name x;             
